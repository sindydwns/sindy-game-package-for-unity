...
Unity를 직접 배치 모드로 실행합니다. `BatchRunner.BuildCommand()`로 명령어 문자열을 생성하거나, `Sindy/Batch/▶ Show Unity Path` 메뉴로 Unity 경로를 확인한 뒤 직접 실행합니다.
...
| 씬 하이라키 읽기 | `ReadSceneHierarchy.Execute` (HTTP IPC) |
