...
| 중간 경로는 명시적으로 패치 (미등록 경로는 예외) | 빈 컨테이너 자동 생성 | 자동 생성은 오타·누락 시 예상치 못한 자식을 조용히 만들어 오류로 번진다 — 미등록 경로는 그 자리에서 예외로 빠르게 실패. 컨테이너는 `Patch("a", "container")`로 먼저 만든다 |
| 호출 형태로 생성/재사용 구분 (`Patch(path, prefab)` vs `Patch(path)`) | 키 존재 여부로 자동 분기(하이브리드) | 런타임 상태로 동작을 추론하면 호출부에서 예측 불가 — 같은 코드가 프리팹 구성에 따라 다르게 동작. 인자 수로 의도를 고정하면 항상 같은 동작, 충돌은 즉시 예외 |
